...
export function scoreLiveOfferingForProfile(offering, profileResult, options = {}) {
  ...
  const selectedPriorities = normalizeSelectedIds(options.selectedPriorities);
  const selectedDealBreakers = normalizeSelectedIds(options.selectedDealBreakers);
  const selectedInterests = normalizeSelectedIds(options.selectedInterests);
  const scoring = scoreProgram(
    ...
    selectedInterests,
    { interestBoostFactor: options.interestBoostFactor }
  );
  const generalScoring = scoreProgram(
    profileResult.profile,
    inferred,
    profileResult.traitConfidence || {},
    selectedPriorities,
  ...
  const generalInferredScore = Math.round(generalScoring.score * 100);
  ...
  const personalScore = hasLinkedScore
    ...
    : inferredScore;
  const generalPersonalScore = hasLinkedScore
    ? Math.round(linkedScore * linkWeight + generalInferredScore * (1 - linkWeight))
    : generalInferredScore;
