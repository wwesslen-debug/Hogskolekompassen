...
const searchColumns = ["title", "provider_name", "description", "city", "degree", "eligibility", "application_code"];
...
function normalizedSql(value) {
  return `BTRIM(REGEXP_REPLACE(TRANSLATE(LOWER(${value}), '${searchTranslateFrom}', '${searchTranslateTo}'), '[^[:alnum:]]+', ' ', 'g'))`;
}

function normalizedColumn(name) {
  return normalizedSql(`COALESCE(${name}, '')`);
}
...
function tokenMatch(column, term, params) {
  ...
  return `${normalizedColumn(column)} ~ ${key}`;
}
...
function buildLiveWhere(filters = {}) {
  ...
  const ids = Array.isArray(filters.ids)
    ? [...new Set(filters.ids.map(Number).filter((id) => Number.isInteger(id) && id > 0))].slice(0, 200)
    : [];

  if (ids.length) clauses.push(`id = ANY(${addParam(params, ids)}::int[])`);
