...
const analyticsQuizModes = new Set(["quick", "full"]);
...
async function ensureSupabaseAnalyticsTables() {
  ...
  await query("CREATE INDEX IF NOT EXISTS idx_hk_analytics_quiz_mode_day ON hk_analytics_quiz_mode_daily(day)");
...
async function maybeCleanupAnalytics() {
  ...
  await query("DELETE FROM hk_analytics_quiz_mode_daily WHERE day < current_date - interval '400 days'");
}

function normalizeAnalyticsQuizMode(value) {
  const mode = String(value || "").trim().toLowerCase();
  if (analyticsQuizModes.has(mode)) return mode;
  return "";
}
...
export async function recordSupabaseAnalyticsEvent(record) {
  ...
  const counts = {
    ...
    starts: ["quiz_started", "start_compass"].includes(event) ? 1 : 0,
    completions: ["quiz_completed", "compass_completed"].includes(event) ? 1 : 0,
    resultViews: event === "view_results" ? 1 : 0,
    applicationClicks: ["external_application_click", "application_click"].includes(event) ? 1 : 0,
    compareEvents: event.startsWith("compare_") ? 1 : 0,
    saveEvents: ["saved_live_program", "save_program", "unsave_program", "saved_list_view"].includes(event) ? 1 : 0,
  };
  ...
  const quizMode = normalizeAnalyticsQuizMode(record?.properties?.quizMode);
  if (quizMode && ["quiz_started", "start_compass", "quiz_completed", "compass_completed"].includes(event)) {
    await query(`
      INSERT INTO hk_analytics_quiz_mode_daily (day, quiz_mode, starts, completions, updated_at)
      VALUES (
        $1::date,
        $2,
        CASE WHEN $3::text IN ('quiz_started', 'start_compass') THEN 1 ELSE 0 END,
        CASE WHEN $3::text IN ('quiz_completed', 'compass_completed') THEN 1 ELSE 0 END,
        now()
      )
      ON CONFLICT (day, quiz_mode) DO UPDATE SET
        starts = hk_analytics_quiz_mode_daily.starts + EXCLUDED.starts,
        completions = hk_analytics_quiz_mode_daily.completions + EXCLUDED.completions,
        updated_at = now()
    `, [day, quizMode, event]);
  }
...
export async function getSupabaseAnalyticsOverview(options = {}) {
  ...
  try {
    ...
    const [daily, previousDaily, events, paths, quizModes] = await Promise.all([
      query(`
        SELECT
          day::text AS day,
      ...
      query(`
        SELECT
          ...
          save_events::int AS "saveEvents"
        FROM hk_analytics_daily
        WHERE day >= current_date - ((($1::int * 2) - 1) * interval '1 day')
          AND day < current_date - (($1::int - 1) * interval '1 day')
        ORDER BY day DESC
      `, [days]),
      query(`
      ...
      query(`
        SELECT
          quiz_mode AS "quizMode",
          SUM(starts)::int AS starts,
          SUM(completions)::int AS completions
        FROM hk_analytics_quiz_mode_daily
        WHERE day >= current_date - (($1::int - 1) * interval '1 day')
        GROUP BY quiz_mode
        ORDER BY (SUM(starts) + SUM(completions)) DESC, quiz_mode
      `, [days]),
    ]);

    const sumTotals = (rows) => rows.reduce((sum, row) => ({
    ...
    const totals = sumTotals(daily.rows);
    const previousTotals = sumTotals(previousDaily.rows);
...
export async function resetSupabaseAnalytics() {
  if (!(await ensureSupabaseAnalyticsTables())) {
    return { configured: false, reset: false };
  }

  await query("TRUNCATE TABLE hk_analytics_daily, hk_analytics_event_daily, hk_analytics_path_daily, hk_analytics_quiz_mode_daily");
  return { configured: true, reset: true };
}
...
function mapLiveOffering(row) {
  ...
  const offering = {
    ...rest,
    description: cleanLiveText(row.description),
    eligibility: cleanLiveText(row.eligibility),
...
export async function getSupabaseLiveSitemapEntries(limit = 5000) {
  if (!(await hasSupabaseLiveDataTables())) return [];
  ...
  const result = await query(`
    ...
    ORDER BY synced_at DESC NULLS LAST, start_date DESC NULLS LAST, provider_name, title
    LIMIT $1
  `, [safeLimit]);
  return result.rows;
}
