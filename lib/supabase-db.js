...
function buildLiveWhere(filters = {}) {
  const clauses = ["(school_type IS NULL OR school_type = 'HS')"];
  const params = [];
...
export async function getSupabaseLiveFilterOptions() {
  ...
  const [periods, cities, providers, kinds] = await Promise.all([
    query("SELECT period FROM susa_education_events WHERE period IS NOT NULL AND (school_type IS NULL OR school_type='HS') GROUP BY period ORDER BY MIN(start_date)"),
    query("SELECT DISTINCT city FROM susa_education_events WHERE city IS NOT NULL AND city != '' AND (school_type IS NULL OR school_type='HS') ORDER BY city"),
    query("SELECT DISTINCT provider_name AS value FROM susa_education_events WHERE provider_name IS NOT NULL AND provider_name != '' AND (school_type IS NULL OR school_type='HS') ORDER BY provider_name"),
    query("SELECT DISTINCT kind FROM susa_education_events WHERE kind IS NOT NULL AND kind != '' AND (school_type IS NULL OR school_type='HS') ORDER BY kind"),
  ]);
  return {
...
export async function getSupabaseLiveDataStatus() {
  ...
  const [counts, periods, lastSync, schoolType] = await Promise.all([
    ...
    `),
    query("SELECT period, COUNT(*)::int AS count FROM susa_education_events WHERE period IS NOT NULL AND (school_type IS NULL OR school_type='HS') GROUP BY period ORDER BY MIN(start_date)"),
