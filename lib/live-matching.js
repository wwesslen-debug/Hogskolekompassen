...
export const LIVE_MATCH_SCHEMA_VERSION = 10;
export const DEFAULT_LIVE_CANDIDATE_LIMIT = 5000;
export const DEFAULT_LIVE_RESULT_LIMIT = 24;

const interestById = Object.fromEntries(educationInterests.map((item) => [item.id, item]));
...
function normalizeSelectedInterestIds(items = []) {
...
function selectedInterestLabel(ids) {
  return ids.map((id) => interestById[id]?.label).filter(Boolean).join(" / ");
}
...
function offeringMatchesInterest(offering, interest) {
  if (!interest) return false;
  if ((interest.categories || []).includes(offering.inferredCategory)) return true;
  ...
  return interestKeywords(interest).some((keyword) => keyword && text.includes(keyword));
}

function offeringMatchesSelectedDirection(offering, selectedInterestIds) {
  return selectedInterestIds.some((id) => offeringMatchesInterest(offering, interestById[id]));
}
...
function compareGeneralOfferings(a, b) {
  return Number(b.generalPersonalScore || b.personalScore || 0) - Number(a.generalPersonalScore || a.personalScore || 0)
    || Number(b.personalScore || 0) - Number(a.personalScore || 0)
    || applicationRank(a) - applicationRank(b)
    || Number(b.matchConfidence || 0) - Number(a.matchConfidence || 0)
    || dateValue(a.startDate) - dateValue(b.startDate)
...
function pickSectionOfferings(offerings, limit, used = new Set(), maxPerCategory = 2) {
...
function buildLiveResultSections(scoredOfferings, selectedInterests) {
  const selectedInterestIds = normalizeSelectedInterestIds(selectedInterests);
  const used = new Set();
  const sections = [];
  const eligible = scoredOfferings.filter((offering) => Number(offering.personalScore || 0) >= 42);
  const pool = eligible.length ? eligible : scoredOfferings;
  const directionLabel = selectedInterestLabel(selectedInterestIds);

  if (selectedInterestIds.length) {
    const directionPool = pool
      .filter((offering) => offeringMatchesSelectedDirection(offering, selectedInterestIds))
      .sort(compareLiveOfferings);
    const offerings = pickSectionOfferings(directionPool, 6, used, 3);

    if (offerings.length) {
      sections.push({
        id: "chosen-direction",
        eyebrow: "Din valda riktning",
        title: directionLabel ? `Inom ${directionLabel}` : "Inom din valda riktning",
        description: "Utbildningar som både ligger nära området du valde och matchar din profil.",
        scoreKind: "directed",
        offerings,
      });
    }
  ...
  const generalPool = [...pool].sort(compareGeneralOfferings);
  const generalOfferings = pickSectionOfferings(generalPool, selectedInterestIds.length ? 6 : 12, used, 2);
  if (generalOfferings.length) {
    sections.push({
      id: "general-match",
      eyebrow: "Generell profilmatchning",
      title: "Bästa generella matchningar",
      description: selectedInterestIds.length
        ? "Räknat utan områdesbonus, så att starka utbildningar utanför din första tanke också kan synas."
        : "Räknat direkt mot din profil och diversifierat så att resultatet inte fastnar i en smal kategori.",
      scoreKind: "general",
  ...
  if (selectedInterestIds.length) {
    const unexpectedPool = pool
      .filter((offering) => !offeringMatchesSelectedDirection(offering, selectedInterestIds))
      .filter((offering) => Number(offering.generalPersonalScore || offering.personalScore || 0) >= 50)
      .sort(compareGeneralOfferings);
    const offerings = pickSectionOfferings(unexpectedPool, 6, used, 2);

    if (offerings.length) {
      sections.push({
        id: "unexpected-match",
        eyebrow: "Oväntade matchningar",
        title: "Utanför din valda riktning",
        description: "Förslag som inte ligger i ditt valda område, men där den generella profilen ändå matchar starkt.",
        scoreKind: "general",
        offerings,
      });
    }
...
export function buildLiveMatchResult(profileResult, candidates, options = {}) {
  ...
  const liveResultSections = buildLiveResultSections(scored, selectedInterests);
