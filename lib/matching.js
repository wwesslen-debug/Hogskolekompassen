...
const CATEGORY_SPECIFICITY_RULES = {
  "Transport & Sjöfart": {
    ...
    eitherTraits: [["affar", "ledarskap"], 0.15],
  },
};
...
function categorySpecificityPenalty(profile, program) {
  ...
  return rule.basePenalty + rule.extraPenalty * (1 - explicitness);
}
...
export function scoreProgram(profile, program, traitConfidence, selectedPriorities, selectedDealBreakers) {
  ...
  const final = Math.max(0, Math.min(1, priorityAdjusted - dealBreakers.total - specificityPenalty));
...
export function buildMatchResult(profileResult, programs, selectedPriorities = [], selectedDealBreakers = []) {
  ...
  const matches = programs
    .map((rawProgram) => {
      ...
      const scoring = scoreProgram(profile, program, traitConfidence, selectedPriorities, selectedDealBreakers);
      return {
        ...program,
  ...
  return {
    ...
    matchModel: {
      ...
      explanation: "Totalmatchningen kombinerar intressen, studiestil, arbetssätt och framtidsmål. Valda prioriteringar kan finjustera resultatet och deal-breakers kan ge tydliga avdrag.",
    },
  };
}
