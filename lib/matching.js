...
const EDUCATION_INTEREST_MAX_BOOST = 0.075;
...
function programInterestText(program) {
...
function interestKeywords(interest) {
  return uniqueItems([
    ...(interest.keywords || []),
    ...(interest.subgroups || []).flatMap((group) => group.keywords || []),
  ]).map(normalizeText).filter(Boolean);
}

function interestCategoryScore(program, interest) {
  let score = (interest.categories || []).includes(program.category) ? 0.72 : 0;

  for (const group of interest.subgroups || []) {
    if (!(group.categories || []).includes(program.category)) continue;
    score = Math.max(score, clamp01(group.weight ?? 0.85));
  }

  return score;
}

function interestKeywordScore(programText, interest) {
  const keywords = interestKeywords(interest);
  if (!keywords.length) return { score: 0, matchedKeywords: [] };

  const matchedKeywords = keywords.filter((keyword) => programText.includes(keyword));
  return {
    score: matchedKeywords.length ? Math.min(1, 0.38 + matchedKeywords.length * 0.22) : 0,
    matchedKeywords: matchedKeywords.slice(0, 3),
  };
}

function interestTraitScore(vector, interest) {
  const entries = Object.entries(interest.traits || {}).filter(([key]) => traitKeys.includes(key));
  ...
  return entries.reduce((sum, [key, weight]) => {
    return sum + clamp01(vector?.[key] ?? 0.5) * Number(weight || 0);
  }, 0) / totalWeight;
}

function bestInterestReason(program, programText, interest, categoryScore, matchedKeywords) {
  const subgroup = (interest.subgroups || []).find((group) => {
    const categoryMatch = (group.categories || []).includes(program.category);
    const keywordMatch = (group.keywords || []).some((keyword) => programText.includes(normalizeText(keyword)));
    return categoryMatch || keywordMatch;
  });
  if (subgroup?.label) return subgroup.label;
  if (matchedKeywords.length) return matchedKeywords[0];
  if (categoryScore) return program.category;
  return interest.label;
}

function scoreEducationInterest(program, interest) {
  const programText = programInterestText(program);
  const categoryScore = interestCategoryScore(program, interest);
  const { score: keywordScore, matchedKeywords } = interestKeywordScore(programText, interest);
  const traitScore = interestTraitScore(program.vector || {}, interest);
  let score = categoryScore * 0.52 + keywordScore * 0.28 + traitScore * 0.2;

  if (!categoryScore && !keywordScore) score *= 0.35;

  return {
    score: clamp01(score),
    reason: bestInterestReason(program, programText, interest, categoryScore, matchedKeywords),
  };
}

function educationInterestBoost(program, selectedInterests, options = {}) {
  const factor = Math.max(0.25, Math.min(2.4, Number(options.interestBoostFactor || 1)));
  const maxBoost = EDUCATION_INTEREST_MAX_BOOST * factor;
  const boostWeight = 0.04 * factor;
  const matched = selectedEducationInterests(selectedInterests)
    .map((interest) => ({ interest, ...scoreEducationInterest(program, interest) }))
  ...
  return {
    total: Math.min(
      maxBoost,
      matched.reduce((sum, item) => sum + item.score * boostWeight, 0)
    ),
    details: matched.slice(0, 3).map(({ interest, score, reason }) => ({
      id: interest.id,
...
export function scoreProgram(profile, program, traitConfidence, selectedPriorities, selectedDealBreakers, selectedInterests = [], options = {}) {
  ...
  const interest = educationInterestBoost(program, selectedInterests, options);
...
export function buildMatchResult(profileResult, programs, selectedPriorities = [], selectedDealBreakers = [], selectedInterests = []) {
  ...
  return {
    ...
    matchModel: {
      ...
      educationInterestBoost: Math.round(EDUCATION_INTEREST_MAX_BOOST * 100),
      explanation: "Totalmatchningen kombinerar intressen, studiestil, arbetssätt och framtidsmål. Eventuella valda vardagsintressen jämförs mot utbildningens kategori, ämnesord, undergrupper och profilvektor och kan ge en liten boost. Prioriteringar kan finjustera resultatet och deal-breakers kan ge tydliga avdrag.",
    },
  };
}
