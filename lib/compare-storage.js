...
function normalizeEntry(item, fallbackKind = "live") {
  ...
  if (typeof item === "number" || typeof item === "string") {
    const id = defaultKind === "program" ? normalizeProgramId(item) : normalizeLiveId(item);
    return id == null ? null : { kind: defaultKind, id };
  }
  ...
  const id = kind === "program"
    ? normalizeProgramId(item.programId ?? item.id)
    : normalizeLiveId(item.offeringId ?? item.id);
